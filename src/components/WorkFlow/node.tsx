...
export function Node({ id, data }: NodeProps) {
  ...
  const currentAgentTheme = agentMap[data.type];
  ...
  const customToolkits =
    data.agent?.tools
      ...
      .filter(Boolean) || [];
  const toolkitLabels =
    currentAgentTheme?.toolkitLabels ||
    (customToolkits.length > 0 ? customToolkits : ['Sin protocolos']);
  ...
  const hasClinicalPreview =
    browserImages.length > 0 ||
    (data.type === 'document_agent' &&
      Boolean(data?.agent?.tasks && data.agent.tasks.length > 0)) ||
    (data.type === 'developer_agent' && terminalTasks.length > 0);

  const renderClinicalPreviewPlaceholder = () => (
    <div
      className={`relative h-[180px] w-full overflow-hidden rounded-2xl border bg-[linear-gradient(180deg,rgba(10,18,32,0.95),rgba(15,23,42,0.82))] ${
        currentAgentTheme?.borderColor || 'border-white/10'
      }`}
    >
      <div
        className={`absolute inset-0 ${
          currentAgentTheme?.bgColor || 'bg-slate-500/10'
        } opacity-70`}
      />
      <div className="bg-white/10 absolute -right-8 -top-8 h-24 w-24 rounded-full blur-3xl" />
      <div className="absolute inset-x-0 top-10 flex items-center gap-2 px-4">
        <div
          className={`rounded-2xl p-2 ${
            currentAgentTheme?.bgColorLight || 'bg-white/5'
          } ${currentAgentTheme?.textColor || 'text-white/80'}`}
        >
          {currentAgentTheme?.icon}
        </div>
        <div className="min-w-0">
          <div className="text-white/90 truncate text-xs font-bold">
            {currentAgentTheme?.previewTitle || 'Panel clinico'}
          </div>
          <div className="text-white/55 line-clamp-2 text-[11px] leading-4">
            {currentAgentTheme?.previewHint ||
              'Vista clínica lista para recibir actividad.'}
          </div>
        </div>
      </div>
        ...
        <div className="mb-3 flex items-end gap-1.5">
          <span
            className={`h-4 w-2 rounded-full ${
              currentAgentTheme?.shapeColor || 'bg-white/10'
            }`}
          />
          <span
          ...
          />
          <span className="bg-white/10 ml-2 h-px flex-1" />
          <span
            className={`h-2.5 w-10 rounded-full ${
              currentAgentTheme?.bgColorLight || 'bg-white/10'
            }`}
          />
        </div>
        <div className="flex flex-wrap gap-2">
          {toolkitLabels.slice(0, 2).map((toolkit, index) => (
            <span
              key={`placeholder-toolkit-${index}`}
              className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${
                currentAgentTheme?.borderColor || 'border-white/10'
              } ${currentAgentTheme?.bgColor || 'bg-white/5'} ${
                currentAgentTheme?.textColor || 'text-white/70'
              }`}
            >
              {toolkit}
            </span>
    ...
    <>
      ...
      >
        <div className="flex w-[342px] shrink-0 flex-col border-y-0 border-l-0 border-r-[0.5px] border-solid border-border-secondary">
          ...
          <div
            ref={toolsRef}
            className="mb-sm flex min-h-4 flex-shrink-0 flex-wrap gap-2 px-3"
          >
            {toolkitLabels.map((toolkit, index) => (
              <span
                ...
                className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${
                  ...
                  currentAgentTheme?.textColor || 'text-text-label'
                }`}
              >
                {toolkit}
              </span>
          ...
          >
            ...
            {!hasClinicalPreview && renderClinicalPreviewPlaceholder()}
          </div>
