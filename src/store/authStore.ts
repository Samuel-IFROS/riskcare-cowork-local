...
const DEFAULT_CLOUD_MODEL: CloudModelType = 'gemini-3-pro-preview';
  ...
  persist(
    (set, get) => ({
      ...
      setWorkerList: (workerList) => {
        ...
        void saveCoworkWorkers(token, workerList).catch((error) => {
          console.warn('Failed to sync workers to Supabase:', error);
        });
      },
