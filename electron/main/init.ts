...
export async function checkToolInstalled() {
  return new Promise<PromiseReturnType>(async (resolve, _reject) => {
    ...
    if (!fs.existsSync(pythonPath)) {
      resolve({
        success: false,
        message: `Python executable missing in venv: ${pythonPath}`,
      });
      return;
    }
    ...
    try {
      await execAsync(
        `"${pythonPath}" -c "import email_validator, fastapi, pydantic; print('ok')"`,
        {
          cwd: getBackendPath(),
          env: {
            ...process.env,
            ...getUvEnv(currentVersion),
      ...
      resolve({
        success: false,
        message: `Backend venv dependency check failed: ${error instanceof Error ? error.message : String(error)}`,
      });
      return;
    }

    resolve({ success: true, message: 'Tools and backend venv are healthy' });
  });
}
...
export async function startBackend(
  ...
  const developmentEnvPath = path.join(app.getAppPath(), '.env.development');
  if (!resolvedServerUrl && devServerUrl) {
    const devProxyEnabled =
      readEnvValue(developmentEnvPath, 'VITE_USE_LOCAL_PROXY') === 'true';
    const devProxyUrl = readEnvValue(developmentEnvPath, 'VITE_PROXY_URL');
    if (devProxyEnabled) {
      resolvedServerUrl = buildLocalServerUrl(devProxyUrl);
      if (resolvedServerUrl) {
        resolvedSource = `dev env file (${developmentEnvPath})`;
      } else {
        log.warn(
          `VITE_USE_LOCAL_PROXY is true in ${developmentEnvPath} but VITE_PROXY_URL is empty or invalid, ignoring`
        );
      }
    }
  }
  ...
  const devSupabaseUrl = resolveConfigValue(
    readEnvValue(developmentEnvPath, 'SUPABASE_URL'),
    readEnvValue(developmentEnvPath, 'VITE_SUPABASE_URL')
  );
  const devSupabasePublishableKey = resolveConfigValue(
    readEnvValue(developmentEnvPath, 'SUPABASE_ANON_KEY'),
    readEnvValue(developmentEnvPath, 'SUPABASE_PUBLISHABLE_KEY'),
    readEnvValue(developmentEnvPath, 'VITE_SUPABASE_PUBLISHABLE_KEY')
  );
  const devSupabaseServiceRoleKey = readEnvValue(
    developmentEnvPath,
  ...
  const supabaseUrl = resolveConfigValue(
    ...
    readEnvValue(globalEnvPath, 'SUPABASE_URL'),
    readEnvValue(globalEnvPath, 'VITE_SUPABASE_URL'),
    devSupabaseUrl
  );
  const supabasePublishableKey = resolveConfigValue(
    process.env.SUPABASE_ANON_KEY,
    process.env.SUPABASE_PUBLISHABLE_KEY,
    process.env.VITE_SUPABASE_PUBLISHABLE_KEY,
    readEnvValue(globalEnvPath, 'SUPABASE_ANON_KEY'),
    readEnvValue(globalEnvPath, 'SUPABASE_PUBLISHABLE_KEY'),
    readEnvValue(globalEnvPath, 'VITE_SUPABASE_PUBLISHABLE_KEY'),
    devSupabasePublishableKey
  );
  ...
  const supabaseWorkersTable = resolveConfigValue(
    process.env.SUPABASE_WORKERS_TABLE,
    readEnvValue(globalEnvPath, 'SUPABASE_WORKERS_TABLE'),
    devSupabaseWorkersTable
  );
  ...
  const env = {
    ...
    VITE_SUPABASE_URL: supabaseUrl || process.env.VITE_SUPABASE_URL || '',
    SUPABASE_ANON_KEY:
