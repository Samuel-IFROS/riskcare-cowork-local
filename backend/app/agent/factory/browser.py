...
def browser_agent(options: Chat):
    ...
    pyautogui_tools = []
    pyautogui_tool_names: list[str] = []
    try:
        pyautogui_toolkit = PyAutoGUIToolkit(options.project_id)
        pyautogui_toolkit = message_integration.register_toolkits(
            pyautogui_toolkit
        )
        pyautogui_tools = pyautogui_toolkit.get_tools()
        pyautogui_tool_names.append(PyAutoGUIToolkit.toolkit_name())
    except Exception as exc:
        logger.warning(
            "PyAutoGUIToolkit not available for browser agent: %s",
            exc,
            exc_info=True,
        )
