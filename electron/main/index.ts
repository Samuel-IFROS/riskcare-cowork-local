...
// In development we keep app data under project dist for easier inspection.
// In packaged apps we must use Electron's default userData (writable, outside app.asar).
if (!app.isPackaged) {
  app.setPath('userData', path.join(MAIN_DIST, '.eigent-data'));
}
...
let localAuthCallbackServer: http.Server | null = null;

const WINDOW_THEME_ICON_LIGHT = 'light';
const WINDOW_THEME_ICON_DARK = 'dark';
const WINDOW_ICON_FALLBACK_NAME = 'favicon.ico';
const WINDOW_ICON_LIGHT_PNG_NAME = 'riskcare-logo-icon.png';
const WINDOW_ICON_DARK_PNG_NAME = 'riskcare-logo-icon-blanco.png';
const WINDOW_ICON_LIGHT_ICO_NAME = 'riskcare-logo-icon.ico';
const WINDOW_ICON_DARK_ICO_NAME = 'riskcare-logo-icon-blanco.ico';

const LOCAL_AUTH_CALLBACK_PORT = 3000;
const LOCAL_AUTH_CALLBACK_PATHS = new Set([
  '/auth/callback',
...
const normalizeAppearanceForWindowIcon = (
  appearance: string | null | undefined
) => {
  if (appearance === WINDOW_THEME_ICON_LIGHT) {
    return WINDOW_THEME_ICON_LIGHT;
  }
...
const resolveWindowIconPath = (
  ...
  const iconCandidates = isWin
    ? [
        path.join(
          VITE_PUBLIC,
          normalizedAppearance === WINDOW_THEME_ICON_LIGHT
            ? WINDOW_ICON_LIGHT_ICO_NAME
            : WINDOW_ICON_DARK_ICO_NAME
        ),
        path.join(
          VITE_PUBLIC,
          normalizedAppearance === WINDOW_THEME_ICON_LIGHT
            ? WINDOW_ICON_LIGHT_PNG_NAME
            : WINDOW_ICON_DARK_PNG_NAME
        ),
      ]
    : [
        path.join(
...
function sendAuthCodeToRenderer(code: string, source: string) {
  if (!code) return;

  if (win && !win.isDestroyed()) {
    log.info(`[AUTH CALLBACK] Forwarding auth code from ${source}`);
    win.webContents.send('auth-code-received', code);
    return;
  }

  const callbackUrl = `eigent://auth?code=${encodeURIComponent(code)}`;
  log.info(
    '[AUTH CALLBACK] Main window not ready, queueing oauth callback URL'
  );
...
function startLocalAuthCallbackServer() {
  ...
  localAuthCallbackServer = http.createServer((req, res) => {
    try {
      const requestUrl = new URL(
        req.url || '/',
        `http://localhost:${LOCAL_AUTH_CALLBACK_PORT}`
      );
      ...
      if (!LOCAL_AUTH_CALLBACK_PATHS.has(pathname)) {
  ...
  localAuthCallbackServer.on('error', (error: any) => {
    const message = error?.message || String(error);
    log.error(`[AUTH CALLBACK] Failed to bind localhost:3000 - ${message}`);
  });

  localAuthCallbackServer.listen(LOCAL_AUTH_CALLBACK_PORT, '127.0.0.1', () => {
    log.info(
      `[AUTH CALLBACK] Listening on http://localhost:${LOCAL_AUTH_CALLBACK_PORT}/auth/callback`
    );
  });
}

async function stopLocalAuthCallbackServer() {
  if (!localAuthCallbackServer) {
    return;
  }

  const server = localAuthCallbackServer;
  localAuthCallbackServer = null;

  await new Promise<void>((resolve) => {
    server.close((error) => {
      if (error) {
        log.error('[AUTH CALLBACK] Error while closing callback server', error);
      } else {
        log.info('[AUTH CALLBACK] Local callback server stopped');
      }
      resolve();
    });
  });
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('set-window-theme-icon', (_event, appearance: string) => {
    applyWindowThemeIcon(appearance);
    return { success: true };
  });
  ...
  ipcMain.handle('export-log', async () => {
    try {
      ...
      const defaultFileName = `riskcare-cowork-${appVersion}-${systemVersion}-${Date.now()}.log`;
...
async function createWindow() {
  ...
  applyWindowThemeIcon(nativeTheme.shouldUseDarkColors ? 'dark' : 'light');
...
const cleanupPythonProcess = async () => {
  try {
    ...
    const userDataPath = app.getPath('userData');
    const portFile = path.join(userDataPath, 'port.txt');
...
app.whenReady().then(async () => {
  ...
  // Local-only OAuth callback listener (Google Auth must return to localhost:3000)
  startLocalAuthCallbackServer();
...
app.on('window-all-closed', () => {
  ...
  if (process.platform !== 'darwin') {
    void stopLocalAuthCallbackServer();
    app.quit();
  }
});
...
app.on('before-quit', async (event) => {
  ...
  try {
    ...
    await cleanupPythonProcess();
    await stopLocalAuthCallbackServer();
