...
class AutomationRecordRequest(BaseModel):
    recording_name: str = Field(min_length=1, max_length=128)
    duration_seconds: float = Field(default=15.0, gt=0, le=600)
    start_trigger_key: str | None = Field(default=None, max_length=64)
    stop_trigger_key: str | None = Field(default=None, max_length=64)
    start_wait_timeout_seconds: float = Field(default=300.0, gt=0, le=3600)
    max_recording_seconds: float = Field(default=600.0, gt=0, le=3600)
    capture_mouse_moves: bool = True
    capture_mouse_clicks: bool = True
    capture_mouse_scroll: bool = True
    capture_keyboard: bool = True
    mouse_move_sampling_ms: int = Field(default=16, ge=0, le=1000)
    mouse_move_min_distance: int = Field(default=3, ge=0, le=200)


class AutomationReplayRequest(BaseModel):
    recording_name: str = Field(min_length=1, max_length=128)
    speed_multiplier: float = Field(default=1.0, gt=0, le=10)
    repeat_count: int = Field(default=1, ge=1, le=50)
    initial_delay_seconds: float = Field(default=2.0, ge=0, le=30)


class AutomationRenameRequest(BaseModel):
...
router = APIRouter()
AUTOMATION_TASK_ID = "manual_automation_toolkit"
AUTOMATION_STORAGE_DIR = (
    Path.home() / ".eigent" / "automation_toolkit" / "captures"
)


def _get_automation_toolkit() -> PyAutoGUIToolkit:
    return PyAutoGUIToolkit(
        api_task_id=AUTOMATION_TASK_ID,
        screenshots_dir=str(AUTOMATION_STORAGE_DIR),
    )


@router.get("/automation/recordings", name="list automation recordings")
async def list_automation_recordings():
    try:
        toolkit = _get_automation_toolkit()
        recordings = toolkit.get_recording_names()
        return {"success": True, "recordings": recordings}
    except Exception as e:
        logger.error(
            "Failed to list automation recordings: %s",
            e,
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to list recordings. Check server logs for details.",
        )


@router.get(
    "/automation/recording/{recording_name}",
    name="get automation recording detail",
)
async def get_automation_recording(recording_name: str):
    try:
        toolkit = _get_automation_toolkit()
        recording = toolkit.get_recording(recording_name)
        ...
        logger.error(
            "Failed to get automation recording detail: %s",
            e,
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail=(
                "Failed to get recording detail. "
                "Check server logs for details."
            ),
        )


@router.post("/automation/record", name="record automation steps")
async def record_automation_steps(payload: AutomationRecordRequest):
    try:
        toolkit = _get_automation_toolkit()
        result = toolkit.record_user_actions(
            recording_name=payload.recording_name,
            duration_seconds=payload.duration_seconds,
            start_trigger_key=payload.start_trigger_key,
            stop_trigger_key=payload.stop_trigger_key,
            start_wait_timeout_seconds=payload.start_wait_timeout_seconds,
            max_recording_seconds=payload.max_recording_seconds,
            capture_mouse_moves=payload.capture_mouse_moves,
            capture_mouse_clicks=payload.capture_mouse_clicks,
            capture_mouse_scroll=payload.capture_mouse_scroll,
            capture_keyboard=payload.capture_keyboard,
            mouse_move_sampling_ms=payload.mouse_move_sampling_ms,
            mouse_move_min_distance=payload.mouse_move_min_distance,
        )
        success = not result.startswith("Error:")
        return {"success": success, "message": result}
    except Exception as e:
        logger.error(
            "Failed to record automation steps: %s",
            e,
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to record steps. Check server logs for details.",
        )


@router.post("/automation/replay", name="replay automation steps")
async def replay_automation_steps(payload: AutomationReplayRequest):
    try:
        toolkit = _get_automation_toolkit()
        ...
        logger.error(
            "Failed to replay automation steps: %s",
            e,
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to replay steps. Check server logs for details.",
        )


@router.put("/automation/recording/rename", name="rename automation recording")
async def rename_automation_recording(payload: AutomationRenameRequest):
    try:
        toolkit = _get_automation_toolkit()
        result = toolkit.rename_recording(
        ...
        logger.error(
            "Failed to rename automation recording: %s",
            e,
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to rename recording. Check server logs for details.",
        )


@router.delete(
    "/automation/recording/{recording_name}",
    name="delete automation recording",
)
async def delete_automation_recording(recording_name: str):
    try:
        toolkit = _get_automation_toolkit()
        result = toolkit.delete_recording(recording_name=recording_name)
        success = not result.startswith("Error:")
        return {"success": success, "message": result}
    except Exception as e:
        logger.error(
            "Failed to delete automation recording: %s",
            e,
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to delete recording. Check server logs for details.",
        )
