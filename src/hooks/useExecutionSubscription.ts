...
export function useExecutionSubscription(enabled: boolean = true) {
  ...
  const hasElectronIPC = useCallback(
    () =>
  ...
  const resolveExecutionSocketBaseURL = useCallback(async () => {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      const baseURL = await resolveExecutionSocketBaseURL();

      if (!baseURL) {
        console.warn(
          '[ExecutionSubscription] Missing websocket base URL, skipping connection'
        );
        setWsConnectionStatusRef.current('disconnected');
        return;
      }
          ...
          switch (message.type) {
            ...
            case 'error':
              ...
              if (
                ...
                message.message?.toLowerCase().includes('forbidden') ||
                message.message?.toLowerCase().includes('unauthorized')
              ) {
                authFailedRef.current = true;
                setWsConnectionStatusRef.current('unsupported');
                ws.close(1008, 'listener-unavailable');
                break;
              }
              toast.error(`Error del monitoreo clinico: ${message.message}`);
      ...
      ws.onclose = (event) => {
        ...
        if (
          event.code === 1008 ||
          event.reason?.toLowerCase().includes('listener-unavailable') ||
          ...
          if (lastCloseTimestampRef.current === now && enabled) {
            if (reconnectAttemptsRef.current < maxReconnectAttempts) {
              ...
              if (hasEverConnectedRef.current) {
                toast.error(
                  'Se perdio la conexion con el monitoreo clinico en tiempo real'
                );
              } else {
                setWsConnectionStatusRef.current('unsupported');
                console.warn(
                  '[ExecutionSubscription] Listener not available in this installation'
                );
              }
            }
          }
    ...
    startPingInterval,
    stopPingInterval,
  ]); // Only depend on enabled and token - primitives that rarely change
  ...
  }, [connect, disconnect, setWsConnectionStatus]);
