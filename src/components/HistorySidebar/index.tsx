...
export default function HistorySidebar() {
  ...
  const editLabel = t('layout.edit');
  const editText = editLabel === 'layout.edit' ? 'Editar' : editLabel;
  ...
  const handleProjectRename = async (
    ...
    const initialName = (currentName || t('layout.new-project')).trim();
    const nextName = window.prompt(
      t('layout.enter-project-name') || 'Project name',
      initialName
    );
    ...
    if (existingProject) {
      projectStore.updateProject(projectId, { name: trimmedName });
    }

    try {
      await proxyFetchPut(
        `/api/chat/project/${projectId}/name?new_name=${encodeURIComponent(trimmedName)}`
      );
    } catch (error) {
                    ...
                    >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent
                          ...
                          sideOffset={6}
                          collisionPadding={12}
                          className="z-[80] min-w-[132px] rounded-lg border border-solid border-dropdown-border bg-fill-default p-1 shadow-perfect"
                        >
                          <DropdownMenuItem
                          ...
                          <DropdownMenuItem
                            className="cursor-pointer rounded-md bg-transparent text-text-cuation"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDelete(project.project_id);
                            }}
                          >
                            <Trash2 size={16} className="text-icon-cuation" />
                            {t('layout.delete')}
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                        ...
                        >
                          <DropdownMenuItem
                            ...
                            onClick={(e) => {
                              ...
                              handleProjectRename(
                                project.project_id,
                                project.project_name || project.last_prompt
                              );
                            }}
                          >
                            <Edit size={16} />
