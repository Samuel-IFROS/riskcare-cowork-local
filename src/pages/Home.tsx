...
const Workflow = lazy(async () => {
  try {
    return await import('@/components/WorkFlow');
  } catch (error) {
    console.error('[Home] Failed to load workflow workspace.', error);
    return {
      default: function WorkflowFallback() {
        return (
          <div className="flex h-full w-full items-center justify-center px-6 text-center text-text-label">
            No se pudo cargar el flujo de trabajo. Recarga la app para
            reintentar.
          </div>
        );
      },
...
function ConnectionStatusIcon({
  ...
  const getStatusTooltip = () => {
    switch (status) {
      case 'connected':
        return 'Monitoreo clinico en tiempo real activo';
      case 'connecting':
        return 'Conectando monitoreo clinico...';
      case 'unhealthy':
        return 'Conexion inestable del monitoreo clinico';
      case 'unsupported':
        return 'Monitoreo clinico en tiempo real no disponible en esta instalacion';
      case 'disconnected':
      default:
        return 'Monitoreo clinico desconectado';
    }
  };
...
export default function Home() {
  ...
  const canReconnectRealtime =
    ...
    wsConnectionStatus === 'unhealthy' ||
    wsConnectionStatus === 'connecting';

  const renderRealtimeStatusAction = () => {
        ...
        <PopoverContent className="w-64 p-4" side="bottom" align="end">
          <div className="flex flex-col gap-3">
            <p className="text-body-sm text-text-body">
              {wsConnectionStatus === 'connecting'
                ? 'Reconectando monitoreo clinico...'
                : 'Reintentar enlace de monitoreo clinico'}
            </p>
            <Button
              variant="primary"
              size="sm"
  ...
  const renderWorkflow = (agents: Agent[]) => (
    <Suspense
      fallback={
        <div className="flex h-full w-full items-center justify-center text-text-label">
          Cargando flujo...
        </div>
      }
    >
  ...
  useEffect(() => {
    const detectAgentFiles = async () => {
      ...
      try {
        ...
        setHasAgentFiles(Array.isArray(files) && files.length > 0);
      } catch (error) {
            ...
            {activeWorkSpace === 'workflow' && (
              <div className="flex h-full w-full flex-1 items-center justify-center">
                <div className="relative flex h-full w-full flex-col">
                  ...
                  <div className="pointer-events-none absolute inset-0 rounded-xl bg-transparent"></div>
                  <div className="relative z-10 h-full w-full">
                    {renderWorkflow(activeTask.taskAssigning || [])}
                  </div>
                </div>
              </div>
            )}
                    ...
                    <div className="flex w-full flex-row items-center justify-start gap-4">
                      ...
                      >
                        ...
                        <MenuToggleItem
                          ...
                          className="w-32"
                          rightElement={renderRealtimeStatusAction()}
                        >
                          {t('triggers.title')}
                        </MenuToggleItem>
                      </MenuToggleGroup>
