...
const BACKEND_VENV_PROBE_TIMEOUT_MS = 15_000;
...
async function checkBackendVenvHealth(version: string): Promise<{
  healthy: boolean;
  reason: string;
}> {
  ...
  if (!fs.existsSync(pythonPath)) {
    return {
      healthy: false,
      reason: `Python executable not found in venv: ${pythonPath}`,
    };
  }
  ...
  return new Promise((resolve) => {
    const probe = spawn(
      ...
      [
        ...
        'import importlib.util as iu;'
          + 'mods=["email_validator","fastapi","pydantic","pyautogui","pynput"];'
          + 'missing=[m for m in mods if iu.find_spec(m) is None];'
          + 'print("venv-ok" if not missing else "missing:" + ",".join(missing));'
    ...
    const finish = (result: { healthy: boolean; reason: string }) => {
      if (!settled) {
        settled = true;
        resolve(result);
      }
    ...
    const timeout = setTimeout(() => {
      ...
      finish({
        healthy: false,
        reason: `Venv health probe timed out after ${BACKEND_VENV_PROBE_TIMEOUT_MS}ms`,
      });
    }, BACKEND_VENV_PROBE_TIMEOUT_MS);
    ...
    probe.on('error', (error) => {
      ...
      finish({
        healthy: false,
        reason: `Failed to start venv health probe: ${error instanceof Error ? error.message : String(error)}`,
      });
    });
    ...
    probe.on('close', (code) => {
      ...
      if (code === 0 && stdout.includes('venv-ok')) {
        finish({ healthy: true, reason: 'ok' });
        return;
      }
      ...
      finish({
        healthy: false,
        reason: `Backend venv dependency probe failed: ${details}`,
      });
    });
  });
}
...
export const checkAndInstallDepsOnUpdate = async ({
  ...
  return new Promise(async (resolve, _reject) => {
    try {
      ...
      if (hasPrebuiltDeps()) {
        const venvHealth = await checkBackendVenvHealth(currentVersion);
        if (!venvHealth.healthy) {
          log.warn(
            '[DEPS INSTALL] Prebuilt dependencies found, but backend venv is unhealthy. Falling back to full install.',
      ...
      const venvHealth = toolsMissing
        ? {
            healthy: false,
            reason: 'Skipped venv health probe because command tools are missing',
          }
        : await checkBackendVenvHealth(currentVersion);
      const backendDepsHealthy = venvHealth.healthy;

      if (!backendDepsHealthy) {
        log.warn('[DEPS INSTALL] Backend venv health check failed', {
          reason: venvHealth.reason,
        });
      }
      ...
      if (
        ...
        } else if (!backendDepsHealthy) {
          log.info(
            '[DEPS INSTALL] Backend venv is unhealthy, reinstalling dependencies...',
            { reason: venvHealth.reason }
          );
        } else {
          log.info(
